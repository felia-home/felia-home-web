# 指示文 10 — 会員登録LP（2ステップ・購入希望条件収集）

## 概要

会員登録を「見込み顧客獲得LP」として本格実装する。

- **STEP 1**: 基本情報（氏名・メール・電話・パスワード）
- **STEP 2**: 購入希望条件（全17項目）→ admin の `member_profiles` に保存

**作業ディレクトリ**: `C:\Users\User\projects\felia-home-web`

---

## 重要: admin API レスポンスの修正

既存の `app/api/members/register/route.ts` と
会員登録フォームで `data.member_id` と書いている箇所を
**`data.member.id`** に修正する。
...
## Step 1: lib/api.ts に購入希望条件保存APIを追加
...
## Step 2: app/api/members/profile/route.ts の作成（プロキシ）

HP → Admin API への購入希望条件保存プロキシ。
...
## Step 3: app/members/register/page.tsx を全面刷新

2ステップ形式のLP。STEP 1完了後にadminへ登録し、
取得した `member.id` を使ってSTEP 2の条件を保存する。
...
確認項目:
- [ ] `/members/register` でLPヒーロー（グリーングラデーション）が表示される
- [ ] STEP 1フォームが表示される
- [ ] STEP 1送信後にSTEP 2に切り替わる（ステップインジケーターが更新される）
- [ ] チェックボックスが選択でき、選択時にグリーンになる
- [ ] 「あとで設定する」でスキップできる
- [ ] `npm run build` エラーなし
...
- [ ] `lib/api.ts` に `saveMemberProfile` / `updateMemberProfile` 追加
- [ ] `app/api/members/profile/route.ts` 作成済み
- [ ] `app/members/register/page.tsx` 2ステップLP に刷新済み
- [ ] `data.member.id` で memberId を取得している（`data.member_id` ではない）
- [ ] `npm run build` エラーなし
