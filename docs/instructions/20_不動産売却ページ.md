# 指示文 20 — 不動産売却ページ（/sell）

## 概要

`/sell` ページを以下の構成で実装する。
PDFの文章を完全コピーして使用すること。
...
/sell
  ...
  ├── 不動産売却の流れ（Step 01〜06）
  ├── 「仲介」と「買取」の違い（比較表）
  ├── よくある質問（アコーディオン・ダークティール背景）
  ├── 不動産売却時の諸費用
  ├── 少しでも高く売るポイント（Point 01〜03・グリーン背景）
  └── 売却査定CTA → /sell/valuation
```

---

## Step 1: components/sell/SellFAQ.tsx の作成
...
npm run build
git add -A
git commit -m "feat: add /sell page with all sections from PDF"
...
- [ ] `app/sell/page.tsx` 作成済み
- [ ] `components/sell/SellFAQ.tsx` 作成済み（アコーディオン）
- [ ] 7つの強み・流れ・仲介と買取比較・FAQ・諸費用・3つのポイント・査定CTAが全て実装
- [ ] PDFの文章が完全コピーされている
- [ ] `npm run build` エラーなし
