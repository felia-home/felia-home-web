...
`/buy` ページを以下の構成で実装する。PDFの文章を完全コピーすること。
...
/buy
  ├── パンくず・タイトル「不動産購入について」
  ├── ヒーロー画像（フルワイド）
  ├── 購入の流れ（前置き文 + Step01〜10 カード形式）
  ├── 住宅取得時の諸費用4つのポイント（グリーン背景）
  ├── 住宅取得時にかかる主な諸費用一覧（テーブル）
  └── オンライン不動産見学サービス（Step01〜04）
```

---

## Step 1: app/buy/page.tsx の作成
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "16px", marginBottom: "32px" }}
            ...
            {ONLINE_STEPS.map((s, i) => (
              <div key={s.num} style={{ display: "flex", alignItems: "flex-start", gap: "8px" }}>
                ...
                <div style={{ border: "1px solid #E5E5E5", borderRadius: "8px", overflow: "hidden", width: "100%" }}>
...
npm run build
git add -A
git commit -m "feat: add /buy page with purchase flow, costs, online viewing service"
...
- [ ] `app/buy/page.tsx` 作成済み
- [ ] 購入の流れ Step01〜10 カード形式
- [ ] 住宅取得時の諸費用4つのポイント（グリーン背景）
- [ ] 諸費用一覧テーブル
- [ ] オンライン不動産見学サービス Step01〜04
- [ ] PDFの文章が完全コピーされている
...
画像は現時点でプレースホルダー表示。
後で `public/images/buy/` に画像を追加して差し替える。
ヒーロー画像のパスは `/images/buy/buyhero.jpg` を想定。
